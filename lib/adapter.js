...
module.exports = (function () {
  ...
  var adapter = {
    ...
    create: function (connection, collection, values, cb) {
      ...
      var pkType = adapter.getPrimaryKeyType(connection, collection);
      //console.log('pk=', pk);
      Object.keys(values).forEach(function (key) {
        values[key] = utils.prepareValue(values[key]);
        if (pk == key && pk == 'id' && pkType !== 'string') {
          identityInsert = true;
          //console.log(pk, '==', key);
        }
      });
    ...
    getPrimaryKeyType: function (connection, collection) {
      var pkType = 'integer';
      Object.keys(connections[connection].collections[collection].definition).forEach(function (key) {
        if (!connections[connection].collections[collection].definition[key].hasOwnProperty('primaryKey')) return;
        var attributeKey = _.find(_.keys(connections[connection].collections[collection].attributes), function(attributeKey) {
          return connections[connection].collections[collection].attributes[attributeKey].columnName === key;
        });
        pkType = connections[connection].collections[collection].attributes[attributeKey].type;
      });
      return pkType;
    },
